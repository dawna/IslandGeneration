class Perlin {
	...
	private static p : number[]; 													// Doubled permutation to avoid overflow
	...
	public perlin(x: number, y :number, z: number) : number {
        ...
        var xi: number = Math.floor(x & 255);								// Calculate the "unit cube" that the point asked will be located in
        var yi: number = Math.floor(y & 255);								// The left bound is ( |_x_|,|_y_|,|_z_| ) and the right bound is that
        var zi: number = Math.floor(z & 255);								// plus 1.  Next we calculate the location (from 0.0 to 1.0) in that cube.
        var xf: number = x - Math.floor(x);								// We also fade the location to smooth the result.
        var yf: number = y - Math.floor(y); 
        var zf: number = z - Math.floor(z);
        var u: number = Perlin.fade(xf);
        var v: number = Perlin.fade(yf);
        var w: number = Perlin.fade(zf);

        var aaa: number, aba: number, aab: number, abb: number, baa: number, bba: number, bab: number, bbb: number;
